%  helper function for Mass matrix using pre-generated mesh
%
...
if numel(node_list_e) == 1
    ...
    area = (1/2*norm(cross(a, b))+1/2*norm(cross(c, d)))/3;
    return
end

% set area to be initially 0
area = zeros(size(node_list_e));
...
for i = 1:size(node_list_e, 1)
    ...
    % Same logic as case 2, but this time I want to use the area of the
    % ring. Since I'm modifying code from getCotan.m, I figured it will be
    % easier if I simply add two triangles at a time, and divide by 2 since
    % every triangle in this ring is double-counted. The /3 part is for
    % taking 1/3 of the triangle area. 
    area(i) = (1/2*norm(cross(a(i,:), b(i,:)))+ ...
                   1/2*norm(cross(c(i,:), d(i,:))))/4/3;
end
